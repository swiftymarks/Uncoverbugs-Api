...
RSpec.describe "Fetch Notification Setting", type: :request do
  describe 'PUT /notification_settings' do
    context 'Update notification settings with valid Bearer Token as Tester' do
      before(:each) do
        ...
        notification_setting_params = { notification_setting: { newsletter: true, notify_new_project: true, notify_work_evaluation: true, notify_events: true } }.to_json        
        put "http://localhost:3000/api/v1/notification_settings", params: notification_setting_params, headers: { 'Content-Type' => 'application/json', 'Authorization' => 'Bearer ' + access_key }        
      end
    ...
    context 'Update notification setting with invalid Bearer' do
      before(:each) do
        ...
        notification_setting_params = { notification_setting: { newsletter: true, notify_new_project: true, notify_work_evaluation: true, notify_events: true}}.to_json        
        get "http://localhost:3000/api/v1/notification_settings", params: notification_setting_params, headers: { 'Content-Type' => 'application/json', 'Authorization' => 'Bearer XXXXXXX' }        
      end
    ...
    context 'Update notification as company' do
      before(:each) do
        signup_params = { user: { first_name: 'Mark Kevin', last_name: 'King', email:'king.mark.kevin@icloud.com', username: 'swiftymarks', password: '1234567890'}, company_profile: { company_name: 'The Company', company_url: 'http://www.google.com' } }.to_json 
        post "http://localhost:3000/api/v1/authentications/signup_as_company", params: signup_params, headers: { 'Content-Type' => 'application/json' }
        access_key = JSON.parse(response.body)["access_key"]
